...
```
/new-teaching-demo <slug>          # draft teaching_brief.md, interview + paraphrase-confirm
/run-teaching-pipeline <slug>      # build (progressive notebook/script) + verify (auto-debugs failures)
/add-teaching-step <slug>          # keep extending it later — any number of times, same day or later
...
security-check pass because step (e) queries a database"). It keeps three
things every other lightweight shortcut would normally drop:
...
`/add-teaching-step` as many times as you want (before class, live during
class, after class to extend for tomorrow). Each call to `/add-teaching-step`:
loads the existing notebook/project and its full step log, clarifies and
paraphrases the new addition back to you, appends it without disturbing
earlier already-verified steps, and re-verifies the whole artifact top to
bottom (a new step can break an earlier one — this is caught, not assumed
away).
...
```text
teaching/<slug>/
  teaching_brief.md   # ordered steps, format (notebook/script), constraints
  notebook.ipynb       # or app.py / steps/step_a_*.py ... — progressive artifact
